...
router.get("/electricStation/searchPort", (req, res) => {
    ...
    SELECT state, COUNT(DISTINCT(sid)) AS numStations, port AS portType FROM elecStationsPort
    GROUP BY state, portType
    HAVING state IN` + ` ` + stateFilter + ` ` +
    `AND port IN`+ ` ` + portFilter +
    `ORDER BY portType ASC, numStations DESC`
    , (err, data) => {
    if (err) {
        console.log(err);
        res.status(500).json({});
...
router.get("/electricStation/searchSpeed", (req, res) => {
    ...
    HAVING state IN` + ` ` + stateFilter + ` ` +
    `AND speedLevel IN`+ ` ` + speedFilter +
    `ORDER BY speedLevel ASC, numStations DESC`
    , (err, data) => {
    if (err) {
        console.log(err);
        res.status(500).json({});
...
router.get("/electricStation/searchNetwork", (req, res) => {
    ...
    `AND network IN`+ ` ` + networkFilter +
    `ORDER BY network ASC, numStations DESC`
    , (err, data) => {
    if (err) {
        console.log(err);
        res.status(500).json({});
